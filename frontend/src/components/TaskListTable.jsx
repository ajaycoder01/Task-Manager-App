    ...
    const getStatusBadgeColor = (status)=>{
     ...
     const getPriorityBadgeColor = (priority)=>{
        switch (priority){
            ...
            case 'Low': return 'bg-green-100 text-green-500 border borde-green-200';
            default: return 'bg-gray-100 text-gray-500 border border-gray-200';
        }
    };
                    ...
                    <tr key={task._id} className='border-t border-gray-200 '>
                        ...
                        <td className='px-4 py-4'>
                            <span className={`px-2 py-1 text-xs rounded inline-block 
                                ${getStatusBadgeColor(task.status)}`}>{task.status}</span>
                        </td>
                        <td className='py-4 px-4'>
                            <span className={`px-2 py-1 text-xs rounded inline-block 
                                ${getPriorityBadgeColor(task.priority)}`}>{task.priority}</span>
                        </td> 
                        <td className='py-4 px-4 text-gray-700 text-[13px] text-nowrap hidden md:table-cell'> {task.createdAt ? 
                                moment(task.createdAt).format('Do MMM YYYY'): "N/A"}
                        </td>
                    </tr>
                ))
            }
